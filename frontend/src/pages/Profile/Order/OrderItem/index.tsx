  ...
  return (
    <div className="order-item">
        ...
        {order.products.map((i) => (
          <div className="order-item-product">
            ...
            <div className="order-item-product-content">
              <Link
                className="order-item-product-content-name"
                to={`../../product/${i.id.slug}`}
              >
                {i.id.name}
              </Link>
              ...
              <div className="order-item-product-content-total">
                <span className="order-item-product-content-total-quantity">
                  Quantity: <b>{i.count}</b>
                </span>
                <span className="order-item-product-content-total-price">
                  Total:{" "}
                  <b>
                    $
                    {(i.count * getDiscountPrice(i.price, i.discount)).toFixed(
                      2
                    )}
                  </b>
                </span>
      ...
      <div className="order-item-content-summary">
        <span>
          Delivery Fee: <b>${order.delivery}</b>
        </span>
        <span>
          Location: <b>{order.location}</b>
        </span>
        <span>
          Ordered At: <b>{formatted}</b>
        </span>
        <span>
          Remaining Time: <b>{remain === "" ? "Finished" : remain}</b>
        </span>
        <span>
          Final price: <b>${order.totalPrice.toFixed(2)}</b>
        </span>
      </div>
    </div>
  );
